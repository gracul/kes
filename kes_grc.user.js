...
var loca = selectLanguage(languageSelector),
    Query = function () {
        for (var pair, query = {}, search = location.search.substring(1).split("&"), i = search.length; i--;) pair = search[i].split("="), query[pair[0]] = decodeURIComponent(pair[1]);
        return query
    }(),
    kes = {},
    l = loca;
    ...
    var privateHelper = {
            ...
            resizeTextarea: function () {
                return this.each(function () {
                    var textareaHeight = $(this).prop("scrollHeight"),
                        windowHeight = .9 * window.innerHeight,
        ...
        kesForUtil = {
            ...
            saveSettings: function (prefix, obj) {
                ...
                for (var i in obj)
                    if (obj[i] instanceof Object)
                        if (prefix.indexOf("highlightgroups") > -1 && "group" == i) {
                            var groups = {};
            ...
            getUrlParameters: function (url) {
                for (var pair, query = {}, search = url.substring(url.indexOf("?") + 1).split("&"), i = search.length; i--;) pair = search[i].split("="), query[pair[0]] = decodeURIComponent(pair[1]);
                return query
            },
            ...
            genericBBCode: function (creator, data) {
                $("body").append('<div id="kes_box"><textarea style="width: 99%; max-height: 90% !important; resize: none;" id="kes_genericBBCode"></textarea></div><div id="kes_overlay" class="kes-backlight"></div>'), $("#kes_genericBBCode").text(creator(data)), $("#kes_overlay").fadeIn().bind("click", function () {
                    $("#kes_overlay, #kes_box").kes("fadeOutRemove")
                }), $("#kes_box").fadeIn(), $("#kes_genericBBCode").kes("resizeTextarea")
            },
            ...
            queue: function (options) {
                ...
                options.complete = function (request, status) {
                    callQueue.shift(), originalCompleteCallback && originalCompleteCallback(request, status), callQueue.length > 0 ? $.ajax(callQueue[0]) : location.reload()
                }, callQueue.push(options), 1 == callQueue.length && $.ajax(options)
            },
...
function calculateTroopScore(playerPage) {
    var playerPage = $(playerPage),
        ...
        troopScoremax = 1e4 * cities,
        startIndex = playerPage.find("table.borderlist").eq(3).find("tr > th:last").parent().index(),
        cityScore = 0;
    return bonusScore = 0 > bonusScore ? 0 : bonusScore, playerPage.find("table.borderlist").eq(3).find("tr:gt(" + startIndex + ")").each(function () {
        cityScore += Number($(this).find("td:last").html().replace(/\./g, ""))
    }), [totalScore - (bonusScore + cityScore), troopScoremax]
}

function displayAttacksWithSeconds(element) {
    function createDate(time) {
        var date = new Date;
        return date.setHours(parseInt10(time[0])), date.setMinutes(parseInt10(time[1])), date.setSeconds(parseInt10(time[2])), date
    }

    function formatDate(date, text) {
        var formattedDate = text;
        return formattedDate = formattedDate.replace(/\d+\.\d+\./, date.getDate() + "." + $.kes("paddedNumber", date.getMonth() + 1) + "."), formattedDate = formattedDate.replace(/[0-9]{2}:[0-9]{2}/, date.getHours() + ":" + $.kes("paddedNumber", date.getMinutes()) + ":" + $.kes("paddedNumber", date.getSeconds()))
    }
    $("table.borderlist").eq(element).find("tr").each(function () {
        ...
        if (cur.find("td:last > span").length > 0) {
            ...
            servertime.setSeconds(servertime.getSeconds() + parseInt(incomming[2], 10)), servertime.setMinutes(servertime.getMinutes() + parseInt(incomming[1], 10)), servertime.setHours(servertime.getHours() + parseInt(incomming[0], 10));
...
function filterOverdueAttacks(attacks) {
    ...
    $.each(attacks, function (villageId) {
        village_attacks = attacks[villageId], tmp_attacks[villageId] = {}, tmp_attacks[villageId].length = 0;
        for (var index in village_attacks)
            if (village_attacks.hasOwnProperty(index)) {
                var single_attack = village_attacks[index],
                    time = parseInt10(single_attack[6]) - Date.parse(new Date) / 1e3;
...
function putSettings(start_tab) {
    function switch_tabs(obj) {
        if (obj.attr("rel")) {
            $(".kes-content-wrapper").hide(), $(".kes-menu a").removeClass("kes-active");
            var id = obj.attr("rel");
            $("#" + id).slideDown(800), obj.addClass("kes-active")
        }
    }
    var html = "";
    html += '<div id="kes_overlay" class="kes-backlight"></div>', html += '<div class="kes-user-settings">', html += '	<a href="#" id="kes_close" class="kes-close">', html += '		<span class="kes-icons kes-icon-close"></span>', html += "	</a>", html += '	<span id="kes_save_success" class="kes-saved">' + l.saved + '<span class="kes-icons kes-icon-saved"></span></span>', html += "	<div>", html += '		<div class="kes-menu-wrapper">', html += '				<span class="kes-heading">Kingsage Enhancement Suite<a href="http://kingsage-enhancement.com/" target="_blank"><span class="kes-icons kes-icon-home"></span></a></span><span class="kes-subheading">version ' + version + "</span>", html += '			<div class="kes-menu">', html += '				<div class="kes-separator"></div>', html += '				<a href="#" id="kes_reset_settings">' + l.resetSettings + "</a>", html += '				<div class="kes-separator"></div>', html += '				<a href="#" rel="content_modules">' + l.enableDisableModules + "</a>", html += '				<a href="#" rel="content_trebuchet">' + l.buildingTrebuchet + "</a>", html += '				<a href="#" rel="content_spy">' + l.linkspy + "</a>", html += '				<a href="#" rel="content_market">' + l.marketSettings + "</a>", html += '				<a href="#" rel="content_troops">' + l.highlighttroops + "</a>", html += '				<a href="#" rel="content_groups">' + l.highlightgroups + "</a>", html += '				<a href="#" rel="content_trooplinks">' + l.trooplinks + "</a>", html += '				<div class="kes-separator"></div>', html += "			</div>", html += "		</div>", html += '		<div id="content_modules" class="kes-content-wrapper"><div class="kes-content-title"><h1>' + l.enableDisableModules + '</h1></div><div class="kes-content"><ul>' + draw.modul(presets.modul, k.modul) + "</ul></div></div>", html += '		<div id="content_trebuchet" class="kes-content-wrapper">', html += '			<div class="kes-content-title"><h1>' + l.buildingTrebuchet + "</h1></div>", html += '			<div class="kes-content">', html += "				" + l.contentTrebuchet + ': <select id="kes_kata_select" style="width: 125px;"> ' + draw.helper.returnOptionsForObject(buildings) + "</select>", html += "			</div>", html += "		</div>", html += '		<div id="content_spy" class="kes-content-wrapper">', html += '			<div class="kes-content-title"><h1>' + l.spy + "</h1></div>", html += '			<div class="kes-content">' + printf(l.contentSpy, '<input id="kes_spylink_amount" type="text" maxlength="5" size="6" value="' + k.spylink_amount + '">') + "</div>", html += "		</div>", html += '		<div id="content_market" class="kes-content-wrapper">', html += '			<div class="kes-content-title"><h1>' + l.marketSettings + "</h1></div>", html += '			<div class="kes-content">', html += "				" + l.marketDefault + ':   <select id="kes_market_d3fault">' + draw.returnMarketDefault(k.market) + "</select>", html += '			<table class="kes-table"><tbody>', html += "				<tr><th><b>" + l.name + "</b></th><th><b>" + l.stone + "</b></th><th><b>" + l.wood + "</b></th><th><b>" + l.iron + "</b></th></tr>", html += "				" + draw.returnMarketInputs(k.market), html += "			</tbody></table>", html += "			</div>", html += "		</div>", html += '		<div id="content_troops" class="kes-content-wrapper"><div class="kes-content-title"><h1>' + l.highlighttroopsLabel + '</h1></div><div class="kes-content">' + draw.highlighttroops(k.units.modul, units) + "</div></div>", html += '		<div id="content_groups" class="kes-content-wrapper"><div class="kes-content-title"><h1>' + l.highlightGroupslabel + '</h1></div><div class="kes-content">' + draw.highlightgroups(k.highlightgroups) + '<br style="clear: left;"></div></div>', html += '		<div id="content_trooplinks" class="kes-content-wrapper"><div class="kes-content-title"><h1>' + l.modul.trooplinks + '</h1></div><div class="kes-content">' + draw.trooplinks() + "</div></div>", html += "	</div>", html += "</div>", $("body").append(html), $("#kes_kata_select, #kes_market_d3fault, #kes_units_off_one_unit, #kes_units_off_two_unit, #kes_units_def_one_unit, #kes_units_def_two_unit").kes("multiCheckBoxes", [k.kata_select, k.market[k.market.d3fault].option, k.units.off.one.unit, k.units.off.two.unit, k.units.def.one.unit, k.units.def.two.unit]), $('input[class*="kes_color"]').keyup(function () {
        ...
        $(".kes-enable, .kes-disable", parent).toggleClass("selected"), $(".checkbox", parent).prop("checked", !$(".checkbox", parent).prop("checked")).trigger("change")
    }), $(".kes-menu a").click(function () {
        switch_tabs($(this))
    }), switch_tabs($('.kes-menu a[rel="' + start_tab + '"]')), setTimeout(initSettings, 50), $("#kes_overlay, #kes_close").bind("click", exitSettings), $("#kes_reset_settings").bind("click", function () {
        q = confirm(l.resetSettings + "?"), q && ($.kes("saveKey", "kes_user_settings", presets), exitSettings(), updateSettings())
    }), $(".kes-user-settings").on("change", "input, select", function () {
        $.kes("saveKey", "kes_user_settings", $.kes("saveSettings", "kes_", presets)), $("#kes_save_success").kes("fadeInfadeOut"), updateSettings()
    })
}

function initSettings() {
    $(".kes-backlight").fadeIn(200), $(".kes-user-settings").slideDown(600)
}

function exitSettings() {
    $(".kes-backlight").fadeOut(600), $(".kes-user-settings").slideUp(600), setTimeout(function () {
...
var draw = {
    helper: {
        ...
        returnUnitsForTrooplinks: function (no) {
            var troops = k.trooplinks[no],
                out = "";
            out += '<ul id="kes_trooplinks_' + no + '">', out += '<li><span class="kes-width">Name:</span><input type="text" id="kes_trooplinks_' + no + '_name"size="5" value="' + troops.name + '" max-length="7"></li>';
        ...
        returnHighlightGroups: function (no) {
            ...
            if ("" != av)
                for (var i in g) g.hasOwnProperty(i) && groups.push(i);
    ...
    trooplinks: function () {
        for (var number = ["0", "one", "two", "three"], o = "", i = 1; 3 >= i; i++) o += '<div class="kes-paragraph" style="float: left; min-width: 170px; width: 25%;">', o += "<h2>KES " + l.troops + " #" + i + '</h2><div class="kes_list">' + this.helper.returnUnitsForTrooplinks(number[i]) + "</div></div>";
        return o
    },
        ...
        for (var modul in unitsettings)
            if (unitsettings.hasOwnProperty(modul)) {
                var checked = unitsettings[modul] ? 'checked="checked"' : "",
                    ...
                    hidden = "";
                ["spy", "count"].indexOf(modul) > -1 && (hidden = "display: none;", listOne = "<option value=" + k.units[modul].one.unit + "></option>", listTwo = '<option value="12"></option>'), o += '<div class="kes-paragraph kes_troops" style="margin: 10px; float: left; width: 35%; min-width: 340px;"><h2>' + l[modul] + '</h2><p class="kes-switch"><label class="kes-enable' + (checked ? " selected" : "") + '"><span>' + l.turnOn + '</span></label><label class="kes-disable' + (checked ? "" : " selected") + '"><span>' + l.turnOff + '</span></label><input class="checkbox" type="checkbox" id="kes_units_modul_' + modul + '" ' + checked + "></p>", o += '<span style="vertical-align: sub;" class="kes-padding">' + l.abbr + ':</span><input style="width: 24px; text-align:center" type="text" id="kes_units_' + modul + '_abbr" size="1" maxlength="1" value="' + k.units[modul].abbr + '">', o += '<span class="kes-padding">' + color + ':</span><input class="kes_color" id="kes_units_' + modul + '_color" type="text" maxlength="7" size="9" value="' + k.units[modul].color + '" style="text-transform: uppercase; background-color: ' + k.units[modul].color + "; color: " + this.helper.getContrast(k.units[modul].color.substring(1)) + ';">' + this.helper.colorValidation() + "<br />", o += '<span class="kes-padding">' + amount + ':</span><input id="kes_units_' + modul + '_one_amount" value="' + k.units[modul].one.amount + '" type="text" size="9" maxlength="5">', o += '<span class="kes-padding" style="' + hidden + '">' + l.unit + ' 1:</span><select id="kes_units_' + modul + '_one_unit" style="' + hidden + '"">' + listOne + "</select><br/>", o += '<span class="kes-padding" style="' + hidden + '">' + amount + ':</span><input id="kes_units_' + modul + '_two_amount" value="' + k.units[modul].two.amount + '" type="text" size="9" maxlength="5" style="' + hidden + '">', o += '<span class="kes-padding" style="' + hidden + '">' + l.unit + ' 2:</span><select id="kes_units_' + modul + '_two_unit" style="' + hidden + '">' + listTwo + "</select></div>"
            }
        return o
    },
...
$('div[id*="lay_castle_widget"] > ul > li').eq(1).append('<a class="widget_icon widget_icon_1" style="cursor: pointer;" id="kes_show_settings">kes</a>'), $("#kes_show_settings").bind("click", function () {
...
if (isNewerVersion($.kes("loadKey", "kes_version"), version)) {
    ...
    if ($.kes("isKey", "kes_user_settings")) try {
        settings = $.kes("loadKey", "kes_user_settings"), updated_settings = iterateSettings(presets, settings)
    } catch (e) {
        window.alert(l.adoptSettings), updated_settings = presets
    } else updated_settings = presets;
    $.kes("saveKey", "kes_user_settings", updated_settings), updateSettings(), $.kes("saveKey", "kes_version", version)
}! function (kes) {
        ...
        fn: function () {
            function getWorldUnitSpeed() {
                $.ajax({
                    ...
                    success: function (data) {
                        var runtime = 1;
                        runtime = parseFloat($(data) .find('table.borderlist') .eq(0) .find('tr:gt(1):lt(2) > td:nth-child(2)') .html(), 10),
                        $.kes('saveKey', 'kes_worldUnitSpeed', runtime)
                    }
                })
            }
            function calculateDistance(source, target) {
                ...
                if (source != null || target != null)
                   distance = Math.sqrt(Math.pow(Math.abs(parseInt10(source.split("|")[0]) - parseInt10(target.split("|")[0])), 2) + Math.pow(Math.abs(parseInt10(source.split("|")[1]) - parseInt10(target.split("|")[1])), 2));
                return Math.round(100 * distance) / 100
            }
            function insertIntoRuntimeCalc() {
                var unitImg = ['/img/units/unit_spy.png',
                               '/img/units/unit_light.png',
                               '/img/units/unit_heavy.png',
                               '/img/units/unit_axe.png',
                               '/img/units/unit_farmer.png',
                               '/img/units/unit_sword.png',
                               '/img/units/unit_ram.png',
                               '/img/units/unit_snob.png'],
                    unitSpeed = [9,10,11,18,20,22,30,35],
                    unitText = ['Szpicle',
                                'Krzyże',
                                'Czarne',
                                'WW',
                ...
                k.modul.insertIntoRuntimeCalc && $("table.borderlist").eq(2).find("tr").each(function () {
                    ...
                    if (cur.find('td[class*="list"]')) {
                        var children = cur.children(),
                            time = children.eq(4) .find('span') .attr('time'),
                            source = children.eq(1) .find('a:last') .html(),
                            target = children.eq(2) .find('a:last') .html()
                            worldUnitSpeed = $.kes("isKey", "kes_worldUnitSpeed"),
                            i = 0;
                        if (children.eq(0).find("img")[0] != null) {
                            for (i;i<unitSpeed.length-1;++i) {
                                if (calculateDistance(source, target)*(unitSpeed[i]/worldUnitSpeed)*60 - time >= 0)
                                    break
                            }
                            ...
                            attack_text = children.eq(0).text()
                            children.eq(0).html('<a target="_blank" align="left" href="/game.php?&s=tools&m=runtime_calculator' + data + av + '"><img src="' + unitImg[i] + '">' + attack_text + '</a>')
                        }
                    }
                })
            }
            ...
            function createQueue(pages) {
                for (var pages = pages / 50 + 1, step = 50, url = "/game.php?s=ally&m=attacks&start=" + av, queue = [], i = 0; pages > i; i++) queue.push(url + i * step);
                return queue
            }
            ...
            if (queueLength = pageQueue.length, $("#kes_allAttacks").bind("click", function () {
                $(this).replaceWith('<span id="kes_allAttacks">' + l.loading + "... (" + (queueLength - pageQueue.length) + "/" + queueLength + ")<br /><br /></span>"), table.find("tr > td").parent().remove();
                var getAttacks = function (page) {
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if ($("table.borderlist").eq(2).append(attacks), page.length > 0) $("#kes_allAttacks").replaceWith('<span id="kes_allAttacks">' + l.loading + "... (" + (queueLength - pageQueue.length) + "/" + queueLength + ")<br /><br /></span>"), getAttacks(page);
                            else {
                                $("#kes_allAttacks").replaceWith('<span id="kes_allAttacks">' + l.loadingFinished + "<br /><br /></span>"), setTimeout(function () {
                                    $("#kes_allAttacks").kes("fadeOutRemove")
                                }, 800), displayAttacksWithSeconds(2), insertIntoRuntimeCalc(), window.timersDown = [], table.prepend('<tr style="height: 35px;"><td colspan="4"><form id="kes_filterOption"><input type="radio" name="filter" value="attacker" id="kes_filterAttacker" class="filter"> ' + l.attacker + '									<input type="radio" id="kes_filterDefender" value="defender" name="filter" checked="checked" class="filter"> ' + l.defender + '<span style="width: 25px; display: inline-block"> </span>' + l.nameCoords + ': <input style="width: 100px;" type="text" id="kes_filterInput"> <span id="kes_attackRowCount"></span></div></td>									<td colspan="1"><span class="click" id="kes_resetFilter">' + arrow + " " + l.reset + "</span></td></tr>"), $.kes("isKey", "kes_lastAttackFilterInput") && $("#kes_filterInput").val($.kes("loadKey", "kes_lastAttackFilterInput"));
                                var filterAttacks = function (event) {
                                    ...
                                    if (event && !(keycodes.indexOf(event.keyCode) > -1)) {
                                        ...
                                        if ("" == filterInput) 8 != event.keyCode && $("#kes_filterInput").css("border", "1px solid red").effect("pulsate", 300, function () {
                                            ...
                                            suffix = 1 != attackRowCount ? " " + l.attacks : " " + l.attack, $("#kes_attackRowCount").text(attackRowCount + suffix)
                                        }
                                    }
                                };
                                $("#kes_filterInput").bind("keyup", filterAttacks), $("#kes_resetFilter").bind("click", function () {
                ...
                $.kes("isKey", "kes_save_attacks") && (display_delete = "inline", filterOverdueAttacks($.kes("loadKey", "kes_save_attacks"))), $('table[class*="borderlist"]').eq(2).prepend('<tr><td colspan="5"><span id="kes_save_attacks" class="click kes_mark">' + l.saveAttacks + '</span> 					<span id="kes_save_attacks_success" style="display: none; color: green;"> ' + l.saved + '!</span> 					<span style="display: ' + display_delete + ';" class="click kes_mark" id="kes_delete_attacks">' + l.resetSavedAttacks + "</td></tr>"), $("#kes_save_attacks").bind("click", function () {
                    var kes_attacks = {};
                    $('table[class*="borderlist"]').eq(2).find('tr > td[class*="list"]').parent().each(function () {
                        ...
                        var off_ally = "-",
                            off_allyId = "-";
                        attack.eq(2).find('a[href*="info_ally"]').html() && (off_ally = attack.eq(2).find('a[href*="info_ally"]').html(), off_allyId = attack.eq(2).find('a[href*="info_ally"]').attr("href"), off_allyId = off_allyId.substring(off_allyId.lastIndexOf("id=") + 3)), "" != av && (def_villageId = def_villageId.replace(av, ""), off_playerId = off_playerId.replace(av, ""), off_villageId = off_villageId.replace(av, ""), off_allyId = off_allyId.replace(av, ""));
                        var attack_timeleft = attack.eq(4).find("span").attr("time");
                        attack_timeleft = Date.parse(new Date) / 1e3 + parseInt10(attack_timeleft), single_attack = {
                            0: off_player,
                            1: off_playerId,
                            2: off_village,
                            3: off_villageId,
                            ...
                            6: attack_timeleft,
                            7: def_attack_name
                        }, kes_attacks.hasOwnProperty(def_villageId) ? (kes_attacks[def_villageId].length = kes_attacks[def_villageId].length + 1, kes_attacks[def_villageId][kes_attacks[def_villageId].length] = single_attack) : (kes_attacks[def_villageId] = {}, kes_attacks[def_villageId].length = 0, kes_attacks[def_villageId][kes_attacks[def_villageId].length] = single_attack)
                    }), $.kes("saveKey", "kes_save_attacks", kes_attacks), $("#kes_save_attacks_success").fadeIn("slow").fadeOut("fast")
                }), $("#kes_delete_attacks").bind("click", function () {
                    $(this).fadeOut("slow"), $.kes("deleteKey", "kes_save_attacks")
                })
            }
        }
    }
    ...
    kes.module.barracks = {
        ...
        fn: function () {
            ...
            if (identifyActiveTab("&m=command")) page.match("sub=send") ? $('select[name*="kata_target"]').val(k.kata_select) : ($('input[id*="send_"]').focusin(function () {
                ...
                $('input[id*="fillOutButton"]').replaceWith('<input id="kes_disband" type="submit" value="(kes) ' + l.fillIn + '">'), $("input#kes_disband").click(function () {
                    var wanted_units = [];
                ...
                cell.find("td:first").html(cellText.replace("-25%", minus25).replace("+25", plus25)), $("#minus25, #plus25").click(function () {
        ...
        fn: function () {
            $('iframe[src*="forum.php"]').load(function () {
                ...
                if (iframeURL.match("s=forum_thread" + av + "&thread_id=")) {
                    ...
                    $(frame).find("div.smallButton:first").before('<div id="kes_save_thread" class="smallButton"><span style="cursor: pointer;">(kes) ' + l.saveThread + "</span></div>"), $(frame).find("#kes_save_thread").bind("click", function () {
                        $(this).fadeOut().fadeIn();
...
css += "#kes_box { background-color: #fff; display: none; border-radius: 5px; position: fixed; width: 500px; left: 50%; top: 5%; margin-left: -250px; padding: 6px; z-index: 200; }", css += ".kes_progressOuter { border:1px solid #CFAB65; overflow: hidden; width:100px; height:10px; background: #CFAB65; }", css += ".kes_progressInner { width: 0px; height: 10px; background: #FFCC6E; }",
function (kes) {
    kes.module.infoally = {
        ...
        fn: function () {
            ...
            cachedTable.append('<tr><td colspan="2"><span class="click" id="kes_getAllyTroopPoints">' + l.allyTrooppoints + "</span></td></tr>"), $("#kes_getAllyTroopPoints").bind("click", function () {
                    ...
                    function getAllPlayerInfos(members, callback) {
                        function callQueue(members) {
                            getPlayerInfo(members, function (members, playerInfo) {
                                ...
                                playerName = playerName.substring(playerName.indexOf(" ") + 1), playerName = playerName.replace(/\(.*\)/, "").trim(), player.push(playerName), player.push(calculateTroopScore(playerInfo)), i++, $("#kes_getAllyTroopPoints").text(l.loading + ": " + i + "/" + len), $("#kes_progress").css({
                                    width: i * (100 / len) + "%"
                                }), allyTroopPoints.push(player), 0 === --left ? ($("#kes_getAllyTroopPoints").parent().parent().remove(), callback(allyTroopPoints)) : callQueue(members)
                            })
                        }
                        var allyTroopPoints = [],
                            len = members.length,
                            left = len,
                            i = 0;
                        $("#kes_getAllyTroopPoints").removeClass("click").text(l.loading + ": " + i + "/" + len).fadeIn().parent().attr("colspan", 0).after('<td><div class="kes_progressOuter"><div id="kes_progress" class="kes_progressInner" maxwidth="100"></div></div></td>'), callQueue(members)
                    }

                    function showData(allyTroopPoints) {
                        function getDataReady(lineStart, lineMiddle, lineEnd, data) {
                            var i, percentage, divider, o = "",
                                len = data.length;
                            for (i = 0; len > i; i++) divider = 0 == data[i][1][1] ? 100 : data[i][1][1], percentage = data[i][1][0] / divider * 100, o += lineStart + data[i][0] + lineMiddle + $.kes("prettyNumber", data[i][1][0]) + " (" + percentage.toFixed(2) + "%)";
                            return o + lineEnd
                        }
                        ...
                        o += "<tr><th>" + l.player + "</th><th>" + l.trooppoints + "</th></tr>", o += getDataReady("<tr><td>", "</td><td>", "</td></tr>", allyTroopPoints), o += "</table><br />", $("table.borderlist").eq(3).before(o), $("#kes_createBB").bind("click", function () {
                            $("body").append('<div id="kes_box"><textarea style="width: 99%; resize: none;" id="kes_data"></textarea></div><div id="kes_overlay" class="kes-backlight"></div>'), $("#kes_data").text(getDataReady("[player]", "[/player] ", "\n", allyTroopPoints)).select(), $("#kes_overlay").fadeIn().bind("click", function () {
                                $("#kes_overlay, #kes_box").kes("fadeOutRemove")
                            }), $("#kes_box").fadeIn(), $("#kes_data").kes("resizeTextarea")
                        })
                    }
                    var members = [];
...
css += 'tr.kes_selected td { background: url("img/layout/bg_table_cell_marked2.jpg") repeat-x scroll 0 0 transparent !important; }', css += ".unselectable { user-select: none; -moz-user-select: none; -khtml-user-select: none; }", css += ".kes_remove_selection { display: inline; background-color: #fff; cursor: pointer; position: relative; float: right; z-index: 150; color: crimson; font-weight: bold; }", css += "#kes_showSelectedSetts { position: fixed; background: #FFF; font-size: 10pt; border-radius: 5px; padding: 5px; z-index: 200; top: 10%; left: 50%; margin-left: -310px; width: 620px; max-height: 85%; overflow-y: auto; }",
function (kes) {
    kes.module.infoplayer = {
        ...
        fn: function () {
            ...
            post_details.std_input = post_details.village, k.modul.bbCodeExport && ($("table.borderlist").eq(3).prepend('<tr><th colspan="3"><span class="click" id="kes_enablebbCodeExport">' + arrow + " (kes) " + l.enableBBCodeExport + "</span></th></tr>"), $("#kes_enablebbCodeExport").bind("click", function () {
                ...
                if ($(this).parent().parent().replaceWith('<tr><th colspan="3">' + l.exportBBCode + '</td></tr><tr><td id="kes_threadId_avail" colspan="3"></td></tr><tr><td colspan="2">					<input type="radio" value="kontinent" name="kes_sel_setts"><span style="vertical-align: bottom; display: inline-block; width: 100px;">' + l.sortContinent + '</span>					<input type="radio" value="koords" name="kes_sel_setts" checked="checked"><span style="vertical-align:bottom">' + l.sortCoords + '</span></td><td></td></tr><td colspan="2">					<input type="radio" value="points" name="kes_sel_setts"><span style="vertical-align: bottom; display: inline-block; width: 100px">' + l.sortPoints + '</span>					<input type="radio" value="name" name="kes_sel_setts"><span style="vertical-align: bottom">' + l.sortName + '</span></td><td><input type="submit" id="kes_sel_setts_submit" value="' + l.bbCode + '"></td></tr>					<tr><td colspan="3"><span class="click kes_mark" id="kes_select_all">' + arrow + " " + l.selectAll + '</span> <span class="click kes_mark" id="kes_deselect_all">' + arrow + " " + l.deselectAll + "</span></td></tr>"), $("table.borderlist").eq(3).prepend(""), $("table.borderlist").eq(3).addClass("unselectable"), $.kes("isKey", "kes_thread")) {
                    ...
                    threadName = thread.name, $("#kes_threadId_avail").html('<span style="color: green; font-weight: bold;">' + l.thread + ' "' + threadName + '" ' + l.selected + ".</span>")
                ...
                $(document).ready(function () {
                    ...
                    $(this).fadeOut().fadeIn(), $("table.borderlist").eq(3).find("tr:gt(5)").each(function () {
                        $(this).selectSettlement()
                    })
                ...
                }).fadeOut().fadeIn(), $("#kes_sel_setts_submit").bind("click", function () {
                    function formatOutput(sett, r) {
                        var o = r;
                        return o = o.replace(new RegExp("{" + post_details.nick + "}", "g"), " " + sett.nick), o = o.replace(new RegExp("{" + post_details.settlementName + "}", "g"), " " + sett.sett), o = o.replace(new RegExp("{" + post_details.village + "}", "g"), " [village]" + sett.pos + "[/village]"), o = o.replace(new RegExp("{" + post_details.continent + "}", "g"), " K" + sett.continent), o = o.replace(new RegExp("{" + post_details.points + "}", "g"), " " + sett.points)
                    }
                    ...
                    function confirmPost(setts, playerName, threadName, threadId, settsPerPosts) {
                        ...
                        $("body").append('<div id="kes_overlay" class="kes-backlight" style="display: block !important;"><h3 style="color: white; text-align:left; opacity: 1; margin: 1em;">' + l.close + '</h3></div>										<div id="kes_showSelectedSetts">											<div style="line-height: 1.1; text-align: left; width: 50%; margin: 0; float: left; border-right: 1px solid #DDD;">												<h3>' + l.summary + ":</h3>												" + l.postIn + ' "' + threadName + '" (<a href="/forum.php?s=forum_thread&thread_id=' + threadId + av + '" target="_blank">' + threadId + "</a>)<br /> " + l.player + ": " + playerName + "<br />												" + l.amountOfSetts + ": " + setts.length + "<br />" + printf(l.postsToBeCreated, postCount) + "<br /><br />												<h3>" + l.formatting + ":</h3>												<h4>" + l.header + ':</h4><textarea style="min-width: 290px; max-width: 290px; min-height: 60px; max-height: 60px;" id="kes_post_top"></textarea>												<h4>' + l.settlementDisplay + ':</h4>												<span class="kes_input click">{' + post_details.nick + '}</span>, <span class="kes_input click">{' + post_details.settlementName + '}</span>, <span class="kes_input click">{' + post_details.village + '}</span>, 												<span class="kes_input click">{' + post_details.continent + '}</span>, <span class="kes_input click">{' + post_details.points + '}</span><br />												<textarea style="min-width: 297px; max-width: 297px; min-height: 90px; max-height: 90px;" id="kes_post_body">{' + post_details.std_input + "}</textarea>												<h4>" + l.footer + ':</h4><textarea style="min-width: 290px; max-width: 290px;min-height: 60px; max-height: 60px; height: 60px;" id="kes_post_footer"></textarea>												<input id="confirm_post_submit" type="submit" value="' + l.confirm + '">											</div>											<div style="line-height: 1.1; text-align: left; padding-left: 3px; width: 49%; margin: 0; float: right;">											<h3>' + l.preview + ':</h3><span id="kes_post_preview"></span>											</div></div>'), updatePreview(), $(".kes_input").bind("click", function () {
                            ...
                            for (var top = $("#kes_post_top").val(), format = $("#kes_post_body").val(), footer = $("#kes_post_footer").val(), output = top + "\n" + playerName + "\n", count = 0, data = [], i = 0; i < setts.length; i++) output += formatOutput(setts[i], format) + "\n", count++, (count == settsPerPosts || i == setts.length - 1) && (output += footer, data.push(output), output = top + "\n" + playerName + "\n", count = 0);
                            makePost(threadId), $(".kes_overlay").click()
                        })
                    }
                    ...
                    function sortKontinents(a, b) {
                        var xya = a.pos.split("|"),
                            xyb = b.pos.split("|"),
                            a = 1e3 * a.continent + (xya[0] + xya[1]),
                            b = 1e3 * b.continent + (xyb[0] + xyb[1]);
                        return a - b
                    }
                    ...
                    if ($.kes("isKey", "kes_thread")) {
                        var thread = $.kes("loadKey", "kes_thread"),
                            ...
                            threadId = thread.id;
                        playerName = $("table.borderlist").eq(2).find("tr:first").text().trim(), playerName = playerName.substring(playerName.indexOf(":") + 1), playerName = $.trim(playerName);
                        var setts = [];
        ...
        fn: function () {
                ...
                destroyList.length > 0 && "" != destroyText && (buildList.before('<span class="click" id="kes_cancelAllDestroy">(kes) ' + l.cancelAllDestroy + "</span><br />"), $("#kes_cancelAllDestroy").bind("click", function () {
                    function cancelDestroy(destroyList, callback) {
                        ...
                        $.ajax({
                            ...
                            url: link,
                            success: function () {
                                that.kes("fadeOutRemove"), 0 == destroyList.length && 1 == buildList.length ? buildList.kes("fadeOutRemove") : callback(destroyList, callback)
                            }
                        })
                    }
                    $(this).kes("fadeOutRemove"), cancelDestroy(destroyList, cancelDestroy)
                }))
            }
            if (k.modul.massbuild && premium) {
                var buildCosts = {
                    ...
                    cost: function (building, level) {
                        var stone = 0,
                            ...
                            ore = 0,
                            workers = 0;
                        return "number" == typeof level && level <= building.max && level >= building.min && (stone = Math.round(building.stone.b * Math.pow(building.stone.f, level - 1)), wood = Math.round(building.wood.b * Math.pow(building.wood.f, level - 1)), ore = Math.round(building.ore.b * Math.pow(building.ore.f, level - 1)), workers = Math.round(building.workers.b * Math.pow(building.workers.f, level - 1))), [stone, wood, ore, workers]
                    },
                    cumulatedCost: function (building, min, max) {
                        for (var stone = 0, wood = 0, ore = 0, workersLow = this.cost(building, min)[3], workersHigh = this.cost(building, max)[3], i = min + 1; max >= i; i++) cost = this.cost(building, i), stone += cost[0], wood += cost[1], ore += cost[2];
                ...
                $('a[href*="&s=build_main&a=buildBuilding"]').each(function () {
                    ...
                    var stone, wood, ore, workers, link, modernStyling = isModernView > 0 ? 'style="color: #F7D48E;" ' : "",
                        ...
                        data = row.find("td");
                    0 == data.length ? (stone = row.find(".res2"), wood = row.find(".res1"), ore = row.find(".res3"), workers = row.find(".workers"), link = row.find(".button")) : (stone = data.eq(1), wood = data.eq(2), ore = data.eq(3), workers = data.eq(4), link = data.eq(6)), stone = stone.find("span").text($.kes("prettyNumber", maximumLevel[1])), wood = wood.find("span").text($.kes("prettyNumber", maximumLevel[2])), ore = ore.find("span").text($.kes("prettyNumber", maximumLevel[3])), workers = workers.find("span").text($.kes("prettyNumber", maximumLevel[4]));
                    var replacement = "<a " + modernStyling + 'href="' + current.attr("href") + '">' + l.buildOne + "</a> ";
                    replacement += '<input type="text" maxlength="2" style="width: 17px;" data-max="' + maximumLevel[0] + '" class="kes_buildLevels" value="' + maximumLevel[0] + '"> ', replacement += "<span " + modernStyling + 'class="click kes_massbuild" data-url="' + current.attr("href") + '">' + l.buildMax + "</span>", current.replaceWith(replacement)
                }), $(".contentpane").on("click", ".kes_massbuild", function () {
                    var url = $(this).data("url");
                    max = $(this).siblings("input").data("max"), levels = $(this).siblings("input").val(), levels = levels > max ? max : levels;
...
css += "#kes_show_attack_info { position: fixed; z-index: 150; box-shadow: 1px 1px 5px #000; display: none; top: 5px; left: 5px; min-width: 250px; max-width: 350px; background-color: #fff; opacity: .9; }", css += "#kes_errorbox { position: fixed; width: 500px; z-index: 150; box-shadow: 1px 1px 5px rgb(0, 0, 0); line-height: 1.2; text-align: left; padding:5px; top: 25px; left: 25px; color: #fff; font-size: 11pt; background-color: #871919; }", css += ".kes_hastroops { position: absolute; padding: 1px; border: 1px solid black; width: 10px; height: 9px; line-height: 8px; font-weight: bold; }", css += "#kes_box { background-color: #fff; display: none; border-radius: 5px; position: fixed; width: 500px; left: 50%; top: 5%; margin-left: -250px; padding: 6px; z-index: 200; }",
function (kes) {
    kes.module.map = {
        ...
        fn: function () {
                ...
                $('td[class*="occupied"] > div > a').each(function () {
                    function hasTroops(troops, units, amount) {
                        var result = !1,
                            unit_1 = units[0],
                            unit_2 = units[1];
                        return 12 == parseInt10(unit_2) ? parseInt10(troops[unit_1].replace(/\./g, "")) > amount[0] - 1 && (result = !0) : parseInt10(troops[unit_1].replace(/\./g, "")) > amount[0] - 1 && parseInt10(troops[unit_2].replace(/\./g, "")) > amount[1] - 1 && (result = !0), result
                    }

                    function drawTroops(elem, troops, modules, pos) {
                        for (var m in modules) modules.hasOwnProperty(m) && modules[m] && hasTroops(troops, [k.units[m].one.unit, k.units[m].two.unit], [k.units[m].one.amount, k.units[m].two.amount]) && elem.append('<div class="kes_hastroops" style="background-color: ' + k.units[m].color + "; color: " + draw.helper.getContrast(k.units[m].color.substring(1)) + "; top: " + pos[m].top + "; right: " + pos[m].right + ';">' + k.units[m].abbr + "</div>")
                    }

                    function isSettlementInGroup(meta, groups) {
                        var result = !1;
                        if (0 != Object.getOwnPropertyNames(groups).length)
                            for (var item in groups)
                                if (-1 != meta.indexOf(groups[item])) {
                                    result = !0;
                                    break
                                }
                        return result
                    ...
                    village = village.substring(village.lastIndexOf("id=") + 3), "" != av && (village = village.replace(av, ""));
                    ...
                    if (k.modul.troopsOnMap && isThisMe && mapOptionActivated) {
                        ...
                        troops = troops.substring(2, troops.length - 1), troops = troops.split(":"), drawTroops($(this), troops, k.units.modul, {
                            def: {
                                top: "2px",
                                right: "2px"
                            },
                    ...
                    if (k.modul.highlightgroups && isThisMe)
                        if (isSettlementInGroup(meta, k.highlightgroups.one.group) && $(this).attr("kes_g_one", k.highlightgroups.one.name), isSettlementInGroup(meta, k.highlightgroups.two.group) && $(this).attr("kes_g_two", k.highlightgroups.two.name), $(this).attr("kes_g_one") && $(this).attr("kes_g_two")) $(this).append('<div style="position: absolute; padding: 1px; border-radius: 5px; border: 1px solid black; background-color: #FF7F50;  bottom: 2px; left: 2px; width: 5px; height: 5px;"></div>');
                        else {
                            var cssMapGroups = "position: absolute; padding: 1px; border-radius: 3px; bottom: 0px; left: 0px;";
                            $(this).attr("kes_g_one") && $(this).append('<div style="' + cssMapGroups + " background-color:" + k.highlightgroups.one.color + "; color: " + draw.helper.getContrast(k.highlightgroups.one.color.substring(1)) + ';"><span>' + $(this).attr("kes_g_one") + "</span></div>"), $(this).attr("kes_g_two") && $(this).append('<div style="' + cssMapGroups + " background-color:" + k.highlightgroups.two.color + "; color: " + draw.helper.getContrast(k.highlightgroups.two.color.substring(1)) + ';"><span>' + $(this).attr("kes_g_two") + "</span></div>")
                        }
            ...
            if (k.modul.troopsOnMap && !mapOptionActivated) {
                $("body").append('<div id="kes_errorbox"></div>'), $("#kes_errorbox").html(0 == $('input[name="map_show_troups"]').length ? l.highlighttroopsError : l.highlighttroopsActivate)
            }
            ...
            $(document).ready(function () {
                function showAttackInformation(villageId) {
                    ...
                    function showNextTwenty() {
                        ...
                        for (var i = 1; size >= i; i++) {
                            var attack_info = village_attacks[i],
                                y = $.kes("prettyTime", parseInt10(attack_info[6]) - Date.parse(new Date) / 1e3);
                            o += '<tr><td><a href="/game.php?s=info_player&id=' + attack_info[1] + av + '" target="_blank">' + attack_info[0] + "</a></td>", o += '<td><a href="/game.php?s=info_village&id=' + attack_info[3] + av + '" target="_blank">' + attack_info[2] + "</a></td><td><b>" + attack_info[7] + "</b></td><td>" + y + "</td></tr>"
                        }
                        return o += "</table>"
                    }
                    var village_attacks = attacks[villageId],
                        ...
                        n = attack_single[7];
                    time = $.kes("prettyTime", parseInt10(time) - Date.parse(new Date) / 1e3);
                ...
                function pulse(element) {
                    $(element).fadeOut(700).fadeIn(700), setTimeout(function () {
                        pulse(element)
                    }, 800)
                }
                if (mapStateChanges(), $('td[title], div[id*="minimap"]').live("click", mapStateChanges), k.modul.showAttacksOnMap && ($("body").append('<div id="kes_show_attack_info">						<table>							<tr><td id="n"></td><td align="right"><span class="click kes_mark" id="kes_close_attack_info">' + l.close + '</span></td></tr>							<tr><td colspan="2"><u>' + l.attacker + "</u></td></tr>							<tr><td>" + l.nick + ': </td><td id="p"></td></tr><tr><td>' + l.settlement + ': </td><td id="v"></td></tr>							<tr><td>' + l.ally + ': </td><td id="a"></td></tr><tr><td>' + l.arrival + ': </td><td id="t"></td></tr>							<tr><td>' + l.moreAttacks + ': </td><td id="m"></td></tr><tr><td id="s" colspan="2"></td></tr>						</table></div>'), pulse('td[attacked*="true"]'), $('td[attacked*="true"]').live("mouseover", function () {
                    ...
                    $("#mapContainer").parent().before('<div id="kes_setGroupsOnMap"><select id="kes_setGroupsOnMap_select">' + options.join() + '</select> <input type="submit" id="kes_setGroupsOnMap_submit" value="' + l.setGroup + '"> <span id="kes_setGroupsOnMap_success" style="display: none; font-weight: bold;">' + l.saved + "</span></div><br />"), $("#kes_setGroupsOnMap_submit").click(function () {
                        ...
                        if (allGroup.hasClass("marked")) {
                            ...
                            $.when(getVillagesOfGroup(groupID)).then(function (raw) {
                                selectedSetts = $(raw).find("input:checked"), selectedSetts.each(function () {
                                    settlements.push($(this).val())
                                }), o = settlements.length, $('td[class*="occupied"] > div > a').each(function () {
                                    var meta = $(this).attr("onmouseover"),
                                        user = meta.split(",")[3],
                                        id = $.kes("getUrlParameters", $(this).attr("href")).id;
                                    user.match(self) && -1 == settlements.indexOf(id) && settlements.push(id)
                                }), n = settlements.length, n != o ? $.ajax({
                                    type: "post",
    ...
    kes.module.market = {
        ...
        fn: function () {
            function calculateRes(avail_donkeys, avail_res, wanted_res) {
                ...
                if (sum <= 1e3 * parseInt10(avail_donkeys))
                    for (var i = 0; 3 > i; i++) out.push(parseInt10(avail_res[i]) > parseInt10(wanted_res[i]) ? wanted_res[i] : avail_res[i]);
                else
                    for (var donkeys = 1e3 * parseInt10(avail_donkeys), scale = donkeys / sum, i = 0; 3 > i; i++) out.push(parseInt10(avail_res[i]) > parseInt10(parseInt10(wanted_res[i]) * scale) ? parseInt10(parseInt10(wanted_res[i]) * scale) : avail_res[i]);
            ...
            if ($.kes("isKey", "kes_lastMarketTarget") && "" == $('input[id="send_x"], input[id="send_y"]').val()) {
                ...
                lastTarget = lastTarget.split("|"), $('input[id="send_x"]').val(lastTarget[0]), $('input[id="send_y"]').val(lastTarget[1])
            }
            ...
            avail_donkeys = avail_donkeys.match(/(\d+)/)[0], calculateRes(avail_donkeys, avail_res, [k.market[k.market.d3fault].stone, k.market[k.market.d3fault].wood, k.market[k.market.d3fault].iron]), $('table[class*="borderlist"]').eq(3).append('<tr><td colspan="3"><span id="kes_market_all"  opt="0" class="click kes_mark">' + arrow + " " + l.all + "</span>" + createOptions(k.market) + "</tr>"), $('span[id*="kes_market_"]').bind("click", function () {
                var wanted_res = [],
                    opt = $(this).attr("opt");
                switch (opt) {
                case "0":
    ...
    kes.module.messages = {
        ...
        fn: function () {
            function forward(type, details, nick) {
                ...
                switch (type) {
                case "report":
                    url = "game.php?s=messages&m=all&a=reportForward&p=" + details.p + "&id=" + details.id, data = "report_to=" + nick;
                    break;
                case "message":
                    url = details.url, data = "msg_to=" + nick + "&confbox=on&" + details.data
                }
                $.ajax({
                    type: "post",
                    url: url + av,
            ...
            if (page.match("&id")) {
                var returnBBCodeForImage = function (a, match) {
                    ...
                    $bbCode = $("#bb_code");
                $bbCode.prevAll(".borderlist:first").find("tr:last > td").append(" " + arrow + ' <span id="kes_report_as_bb_code" class="click">(kes) ' + l.report_as_bb_code + "</span>"), $("#kes_report_as_bb_code").click(function () {
                    bb = $bbCode.html(), bb = bb.replace(regex, returnBBCodeForImage), $bbCode.html(bb).show()
                })
                ...
                $("table.borderlist").eq(5).append('<tr><td colspan="3"><span style="font-weight:bold;">' + l.recipient + ': </span><input type="text" id="kes_massforward_nick"> <input type="submit" id="kes_massforward" value="(kes) ' + l.forward + '"> <span style="display: none; font-weight: bold;" id="kes_massforward_text"></span></td></tr>'), $("#kes_massforward").bind("click", function (event) {
                    event.preventDefault();
                    var table = $("table.borderlist").eq(5);
                    reports = table.find('img[src*="red"], img[src*="green"],img[src*="blue"],img[src*="yellow"]').siblings("input:checked"), messages = table.find('img[src*="mail"]').siblings("input:checked"), nick = $("#kes_massforward_nick").val(), session = "", 0 == reports.length && 0 == messages.length || "" == nick ? $("#kes_massforward_text").html(l.forwardError + "!").css("color", "red").fadeIn(500).delay(800).fadeOut(100) : ($(reports).each(function () {
                        var id = $(this).val();
                        "" == session && (session = reportSession(id)), session.then(function (results) {
                            dt = {}, dt.id = id, dt.p = $.kes("getUrlParameters", $(results).find("form").attr("action")).p, forward("report", dt, nick)
                        })
                        ...
                        dt.then(function (results) {
                            var form = $(results).find("form"),
                                details = {};
                            details.url = form.prop("action"), details.data = form.find('input[type="checkbox"][name="mid[]"]').serialize(), forward("message", details, nick)
                        })
                ...
                formGenerator = function (groups) {
                    ...
                    for (var g in groups)
                        if (groups.hasOwnProperty(g)) {
                            ...
                            o += '<span class="kes_group_element"><input name="vg[]" value="' + groups[g].id + '" type="checkbox" ' + checked + "> " + groups[g].name + " </span>", groups[g].checked && currentGroups.push(groups[g].name)
                        }
                    return o += "</form></td></tr></table><br />", currentGroups.length > 0 && place.text(label + currentGroups.join(", ")), groups.length > 0 ? o : ""
                };
            insert.before(formGenerator(groups)), $("#kes_groups").delegate("input", "click", function () {
                var data = $("#kes_groups"),
                ...
                $.ajax({
                    ...
                    data: data.serialize(),
                    success: function () {
                        $("#kes_groups_savestatus").kes("fadeInfadeOut"), place.text(label + newGroups.replace(/  /g, ", "))
                    }
                })
            })
        }
...
css += ".kes_multiselect { display: none; }", css += ".kes_multiselect_open { display: block !important; }",
function (kes) {
        ...
        fn: function () {
            function getWorldRuntime() {
                $.ajax({
                    ...
                    success: function (data) {
                        var runtime = 1;
                        runtime = parseFloat($(data).find("table.borderlist").eq(0).find("tr:gt(1):lt(1) > td:nth-child(2)").html(), 10), $.kes("saveKey", "kes_worldRuntime", runtime)
                    }
                })
            }
            ...
            function createBarracksString(troops) {
                for (var output = "", labels = ["farmer", "sword", "spear", "axe", "bow", "spy", "light", "heavy", "ram", "kata", "snob"], i = 0; i < labels.length; i++) output += "&" + labels[i] + "=" + troops[i];
                return output
            }

            function getSlowestUnit(troops) {
                for (var units = [20, 22, 22, 18, 18, 9, 10, 11, 30, 30, 35], i = 0; i < units.length; i++) 0 == troops[i] && (units[i] = 0);
            ...
            if (k.modul.filterOverview && identifyActiveTab("s=overview_villages&m=1")) {
                ...
                ui += '<tr><td><span id="kes_filterTroops_status">' + l.troopFilter + "</span></td>" + head + "</tr>", ui += '<tr><td><input id="kes_filterTroops"  value="' + l.filter + '" type="submit"></td><td><input type="text" name="farmer" size="4" maxlength="5"></td>', ui += '<td><input type="text" name="sword" size="4" maxlength="5"></td><td><input type="text" name="spear" size="4" maxlength="5"></td>', ui += '<td><input type="text" name="axe" size="4" maxlength="5"></td><td><input type="text" name="bow" size="4" maxlength="5"></td>', ui += '<td><input type="text" name="spy" size="4" maxlength="5"></td><td><input type="text" name="light" size="4" maxlength="5"></td>', ui += '<td><input type="text" name="heavy" size="4" maxlength="5"></td><td><input type="text" name="ram" size="4" maxlength="5"></td>', ui += '<td><input type="text" name="kata" size="4" maxlength="5"></td><td><input type="text" name="snob" size="4" maxlength="5"></td></tr>', ui += "<tr><td>" + l.target + ': <input id="kes_filterTroops_target" type="text" size="7"></td>', ui += '<td colspan="10">' + l.toa + ": ", ui += l.days + ' <input id="kes_filterTroops_arrival_d" type="text" size="2">' + l.hours + ' <input id="kes_filterTroops_arrival_h" type="text" size="2">', ui += l.minutes + ' <input id="kes_filterTroops_arrival_m" type="text" size="2"> ' + l.seconds + ' <input id="kes_filterTroops_arrival_s" type="text" size="2">', ui += ' <input type="checkbox" id="kes_filterTroops_onlyTroops">' + l.withoutRuntime + "</td>", ui += '<td><input type="submit" id="kes_filterTroops_save" value="' + l.save + '"></td>', ui += '</tr><tr><td colspan="12">' + displayTrooplinks() + "</td></tr></table><br />", $("table.borderlist").eq(3).before(ui), $.kes("isKey", "kes_worldRuntime") || getWorldRuntime();
                ...
                $.kes("isKey", "kes_filterTroops") && (filterTroopsInput = $.kes("loadKey", "kes_filterTroops")), $("#filterTroops_table").find("input:gt(0):not(:last)").each(function (i) {
                    ...
                    $.kes("saveKey", "kes_filterTroops", getInput()), $("#kes_filterTroops_status").fadeOut().html(l.saved).css("color", "green").fadeIn("slow", function () {
                        $(this).delay(800).html(l.troopFilter).css("color", "black").fadeIn()
                    })
                }), $("#kes_filterTroops").bind("click", function () {
                    $("table.borderlist").eq(4).find("tr:first > th:nth-child(2)").html(l.toa), parseInt10($("#kes_filterTroops_noMatch").length) > 0 && $("#kes_filterTroops_noMatch").remove();
                    var isPaginated = !1,
                        currInput = getInput(),
                        ...
                        troops = [currInput[0], currInput[1], currInput[2], currInput[3], currInput[4], currInput[5], currInput[6], currInput[7], currInput[8], currInput[9], currInput[10]],
                    ...
                    $("table.borderlist").eq(4).find("tr:gt(0)").each(function () {
                        if ($(this).find("td:first").attr("colspan")) isPaginated = !0, $(this).addClass("pagination");
                        else {
                            ...
                            if (onlyTroops || time > runtime) {
                                ...
                                if ($(this).find("td:gt(4)").each(function (i) {
                                    var unit;
                                    10 > i ? (unit = $(this).attr("title"), unit = unit.substring(0, unit.indexOf(" ")), unit = unit.replace(/\./g, "")) : unit = $(this).find("a").html(), avail_troops.push(unit)
                                }), checkForTroops(troops, avail_troops)) {
                                    var id = $(this).find("td:first").find("a").attr("href");
                                    id = id.substring(0, id.lastIndexOf("&s=") + 3), $(this).attr("time", runtime).css("display", "table-row").find("td:nth-child(2)").html('<a class="kes_link_to_barracks" href="' + id + "build_barracks&m=command" + barracksString + "&send_x=" + target[0] + "&send_y=" + target[1] + av + '" target="_blank">' + $.kes("prettyTime", runtime) + "</a>"), sortable.push(runtime)
            ...
            } else if (identifyActiveTab("s=overview_villages&m=8")) $(".contentpane .borderlist").eq(1).find("th").parent().before('<tr><td colspan="13"><span id="kes_research" class="click">' + l.researchMissingTroops + "</span></td></tr>"), $("#kes_research").click(function () {
                $(this).text(l.beingProcessed + "...").css({
                    color: "crimson",
                    "font-weight": "bold"
                });
                var session, research = [];
                $(".contentpane .borderlist").eq(1).find('img[src*="grey.png"]').parent().parent().each(function () {
                    ...
                    villageId = villageId.substring(villageId.indexOf("village=") + 8, villageId.indexOf("&s=build_barracks")), current.find("td:gt(0):lt(10)").each(function (i) {
                    ...
                    $.ajax({
                        ...
                        success: function () {
                            if (queue.length > 0) makeResearch(queue);
                                ...
                                page = "undefined" == typeof page ? "" : "&start=" + page, $.ajax({
                                    type: "post",
                                    url: "/game.php?s=overview_villages&m=8" + page + av,
                                    success: function (data) {
                                        var table = $(data).find(".contentpane .borderlist").eq(1);
                                        $(".contentpane .borderlist").eq(1).replaceWith(table), window.alert(l.researchStarted)
                                    }
                                })
                            }
                        }
                ...
                if ($('table[class*="borderlist"]').eq(3).find('input[type="checkbox"]').parent().parent().each(function () {
                    ...
                    villageId = villageId.substring(villageId.lastIndexOf("=") + 1), $(this).find("td:first").attr("colspan", "1").after('<td class="marked_group"><span class="click removeall" villageId="' + villageId + '">' + l.all + "</span></td>")
                    ...
                    $("#kes_loadSelection").bind("click", function () {
                        for (var i in selection) selection.hasOwnProperty(i) && $('table[class*="borderlist"]').eq(3).find('td[class*="marked_group"]').parent().find('td:first > a[href*="' + selection[i] + '"]').each(function () {
                            $(this).siblings("input").attr("checked", "true"), $(this).parent().parent().find('td[class*="marked_group"]').eq(1).find("span").removeClass("click removeall")
                        })
                    }), $('td[colspan*="14"]').append(' <span id="kes_deleteSelection" class="click kes_mark">' + l.deleteSelection + "</span>"), $("#kes_deleteSelection").bind("click", function () {
                        $.kes("deleteKey", "kes_selection"), $("#kes_loadSelection, #kes_deleteSelection").kes("fadeOutRemove")
                    })
                }
                ...
                table.before(button), button.click(function () {
                    $(this).remove(); {
                        var isPaginated = 14 == table.find("tr:first > td").prop("colspan"),
                            bounds = isPaginated ? 2 : 1;
                        table.find("th:first").parent()
                    }
                    rows = table.find("tr:gt(" + (bounds - 1) + ")"), rows = rows.slice(0, -bounds), rows.map(function (i, element) {
                    ...
                    rows.remove(), table.find("tr").eq(bounds - 1).after(sorted);
                    var create_overview = function (sorted) {
                        ...
                        sorted.each(function () {
                            ...
                            arrival = td.eq(3).text(), jump = "#" + $(this).prop("id"), isSnob = 0 != td.eq(14).text(), object[target] ? (object[target].count += 1, isSnob && (object[target].snob += 1, "" == object[target].snobf && (object[target].snobf = arrival, object[target].snobj = jump)), object[target].last = arrival, object[target].llink = barracks) : object[target] = {
                                count: 1,
                                snob: isSnob ? 1 : 0,
                    ...
                    0 == $("#kes_sort_attack_overview").length ? table.before('<table class="borderlist" style="width:820px;" id="kes_sort_attack_overview"><tr><th>' + l.target + "</th><th>" + l.amount + "</th><th>" + l.arrival + "</th><th>" + l.arrival + "</th></tr></table><br />") : $("#kes_sort_attack_overview").find("tr:gt(1)").remove(), $("#kes_sort_attack_overview").append(create_overview(sorted)).on("click", ".jump", function () {
                        var row = $(this).closest("tr"),
                            first = row.find(".jump:last").data("href");
                        amount = row.find(".count").text(), table.find("td.kes_used").removeClass("kes_used"), $(first).prev().nextAll("tr:lt(" + amount + ")").find("td").addClass("kes_used")
                    })
                })
                ...
                if (identifyActiveTab("m=6&type=notignored")) {
                    ...
                    table.parent().each(function () {
                        ...
                        attId = $.kes("getUrlParameters", t.find("a").attr("href")).id, id = $.kes("getUrlParameters", id).id, t.find("a").eq(0).before('<input type="checkbox" class="kes_help_ignore" data-sid="' + id + '" value="' + attId + '">')
                    }), table.parent().parent().append('<tr><th colspan="3"><input type="checkbox" class="kes_help_ignore" id="kes_ignore_all"> ' + l.selectAll + '</th><td colspan="2"><input id="kes_ignore_submit" type="submit" value="' + l.ignore + '"></td></tr>'), $("#kes_ignore_all").click(function () {
                        ...
                        if (checked) {
                            ...
                            $(".kes_help_ignore:checked").each(function () {
                                var t = $(this),
                                    id = t.data("sid"),
                                    attId = t.val();
                                ignore[id] || (ignore[id] = []), ignore[id].push(attId)
                            });
                ...
                submitForm(form.parent()), form.parent().siblings("span.click").find(".kes_multiselect_count").text(count), siblings.eq(1).text(count), siblings.eq(2).html('<div style="width: 200px;">' + $.map(form.find(".checkbox:checked").next("span"), function (element) {
                    return $(element).text()
                }).join("; ") + "</div>")
            }), $(document).ready(function () {
                function lazyManipulate(doubleCache) {
                    doubleCache.slice(step, step + stepSize).each(function () {
                        var parent = $(this).parent(),
                            ...
                            markedGroups = 1 == markedGroups.find("span.notice").length ? [] : $(this).parent().find("td:eq(2)").text().trim().split("; ");
                        villageId = parent.find("td:eq(0)").find("a").attr("href"), villageId = villageId.substring(villageId.indexOf("village=") + 8, villageId.indexOf("&s=overview")), $(this).replaceWith("<td>" + formGenerator(markedGroups, groups, villageId) + "</td>")
                    }), step += stepSize, steps--, steps > 0 && setTimeout(function () {
    ...
    kes.module.runtimecalculator = {
        ...
        fn: function () {
            if (page.match("&target=")) {
                ...
                $.kes("saveKey", "time", time), $.kes("saveKey", "starttime", starttime), $('input[id*="start"], input[id*="target"]').kes("multiCheckBoxes", source.concat(target)), $('input[type*="submit"]').click()
            } else if (page.match("&inta=calculate")) {
                var time = $.kes("deleteKey", "time");
                starttime = $.kes("deleteKey", "starttime"), currenttime = (new Date).getTime(), time -= Math.floor((currenttime - starttime) / 1e3), time = 0 >= time ? 0 : time, time = $.kes("prettyTime", time), $('table[class*="borderlist"]').eq(4).find("tbody").prepend('<tr><td colspan="3"><p style="color: #DC143C; font-weight: bold; text-align: center;">' + l.runtimecalc + " " + time + "</p></td></tr>"), $('table[class*="borderlist"] > tbody > tr > td[class*="right"]').each(function () {
                    ...
                    runtimeInSeconds = inSeconds(runtime_unit), timeInSeconds = inSeconds(timeInSeconds), timeInSeconds > runtimeInSeconds && $(this).css("color", "grey")
                })
            }
        }
    }
...
injectCss.type = "text/css", injectCss.appendChild(document.createTextNode(css)), h.appendChild(injectCss);
